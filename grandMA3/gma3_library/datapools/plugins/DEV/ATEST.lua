--[[ grandMA3 Dimmer Phaser Engines creator v 1.1]]
...
--------------------------------------------------------------------
local DIM_EFFECTS_ENGINE_CREATOR = select(3,...)
local C = Cmd
local TI = TextInput
local E = Echo
...
local function checkifdimmerfixture(group,DP)
...
function DIM_EFFECTS_ENGINE_CREATOR.get_groups(DP)
    local DONE = '[DONE]'
    local userInput
    local cp_groups = {}
    local gp_i = 1
        ...
        if userInput ~= '' and userInput ~= nil then
            if userInput ~= DONE then
                ...
                elseif type(userInput) ~= 'table' then
                    if checkifdimmerfixture(userInput,DP) then
                    ...
                    gp_i = gp_i + 1
                        ...
                        Confirm('Not suitable Group','Group is empty of not all the fixtures in the group have Dimmer.')
                    end
                elseif type(userInput) == 'table' then
                    local dimmersuitable = true
                    for ind = 1, #userInput do
                        local exists
                        if checkifdimmerfixture(userInput[ind],DP) then
                           ...
                           dimmersuitable = false
                        end
                    end
                    if not dimmersuitable then
                        Confirm('Not suitable Groups','One ore more Groups from the range are empty or not all fixtures in these Groups have Dimmer. Groups were deleted from the range.')                         
                    end
                end
            else
            end
...
function DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(object_amount,pool)
    local object_num = 1
    local i = 1
...
function DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num(message,suggested,object_amount,pool)
    local result = false
...
function DIM_EFFECTS_ENGINE_CREATOR.create_name_prefix(DP)
    ...
    local prefix = 'DPE'..tostring(prefix_index)..'_'
    local obj_i = 1
    local pools_i = 1
    ...
    repeat
        repeat
            if #pools[pools_i]:Children() ~= 0 and string.match(pools[pools_i]:Children()[obj_i].name,prefix) then
                ...
                detected = true
                prefix = 'DPE'..tostring(prefix_index)..'_'
                obj_i = 1
            end
            obj_i = obj_i + 1
...
function DIM_EFFECTS_ENGINE_CREATOR.declare_timing_values()
    local fades = {}
        fades[1] = 0
        fades[2] = 1
        fades[3] = 2
        fades[4] = 4
        fades[5] = '(Dimmer Engine X Fade)'
        fades['appearanceR'] = 0
    ...
    local delays_from = {}
        ...
        delays_from[3] = 2
        delays_from[4] = 4
        delays_from[5] = '(Dimmer Engine X Delay-from)'
    ...
    local delays_to = {}
        ...
        delays_to[3] = 2
        delays_to[4] = 4
        delays_to[5] = '(Dimmer Engine X Delay-to)'
    ...
    local groups = {}
        ...
        groups[3] = 3
        groups[4] = 4
        groups[5] = '(Dimmer Engine X Groups)'
    ...
    local blocks = {}
        ...
        blocks[3] = 3
        blocks[4] = 4
        blocks[5] = '(Dimmer Engine X Blocks)'
...
function DIM_EFFECTS_ENGINE_CREATOR.declare_filenames()
    local filenames = {
...
function DIM_EFFECTS_ENGINE_CREATOR.create_appearances(selected_presets, first_appear,filenames,in_declared_timings,choise,naming_prefix,progress_bar_time,DP)
    --[[
        Function to create appearances
    ]]--
    if Version() == '1.9.7.0' then
        C('cd 13.2.2')
        C('call library')
        C('cd root')
    ...
    local forms_filenames_active = {'sine_black.png','snapin_black.png','snapout_black.png','pwm_black.png'}
    local forms_filenames = {'sine_white.png','snapin_white.png','snapout_white.png','pwm_white.png'}
    local forms_names = {'form_sine','form_snapin','form_snapout','form_pwm'}
...
function DIM_EFFECTS_ENGINE_CREATOR.import_predefined_phasers(all1_presets_pool,first_preset,naming_prefix)
    local predefined_import_num = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(111,all1_presets_pool) -- 111 for 107 predefined presets and four presets for sin, snapin,snapout,pwm
    C('import preset 21.'..predefined_import_num..' /File "predefined_phaser.xml" /nu')
    local sin_phaser_num = predefined_import_num + 1 - 1
    local snapin_phaser_num = predefined_import_num + 2 - 1
    local snapout_phaser_num = predefined_import_num + 3 - 1
    local pwm_phaser_num = predefined_import_num + 4 - 1
    C('copy preset 21.'..sin_phaser_num..' + '..snapin_phaser_num..' + '..snapout_phaser_num..' + '..pwm_phaser_num..' at 21.'..(predefined_import_num + 107)..' /nu')
    ...
    all1_presets_pool[first_preset]:Set('name',naming_prefix..'sine')
    all1_presets_pool[first_preset+1]:Set('name',naming_prefix..'snapin')
    all1_presets_pool[first_preset+2]:Set('name',naming_prefix..'snapout')
    all1_presets_pool[first_preset+3]:Set('name',naming_prefix..'pwm')
end

function DIM_EFFECTS_ENGINE_CREATOR.create_matricks(first_matricks,naming_prefix,index)
    C('store matricks '..first_matricks..' "'..naming_prefix..index..'_phaser_engine"'..' /nu')
end

function DIM_EFFECTS_ENGINE_CREATOR.create_sequence(first_sequence,group_amount,first_appearance,first_preset,first_matrick,sequence_pool,selected_page,naming_prefix,index,DP)
    local appearances_pool = ShowData().appearances
    ...
    local encoder_executor = ObjectList('page '..selected_page..'.'..(executor+200))[1]
    encoder_executor:Set('fader','Speed')
    encoder_executor:Set('key','DoubleSpeed')
end

function DIM_EFFECTS_ENGINE_CREATOR.create_macros(groups,first_macro,first_sequence,first_matrick,first_appearance,first_preset,in_declared_timings,naming_prefix,index,DP)
    local macro_pool = ShowData().datapools[DP].Macros
    local appearances_pool = ShowData().Appearances
    local groups_pool = ShowData().DataPools[DP].Groups
    local sequence_pool = ShowData().datapools[DP].Sequences
    ...
    local sin_form_macro_name = macro_pool[first_macro+#groups].name
    ...
    local matricks_macros_names = {sin_form_macro_name,selection_in_order_macro_name,no_mirror_macro_name,phase_0_macro_name,group_0_macro_name,blocks_0_macro_name,fade_0_macro_name,delay_from_0_macro_name,delay_to_0_macro_name}
    macro_pool[macro_ind]:Set('name',macro_name)
...
function DIM_EFFECTS_ENGINE_CREATOR.create_delete_macro(first_macro,in_last_macro,first_sequence,number_of_presets,amount_of_groups,dpe_layout,first_appear,in_last_appear,first_preset,first_matrx,presets,choise,index,name_prefix,DP)
    ...
    local line3 = ''
    local line4 = 'Delete Layout '..dpe_layout..' /nc /nu'
    local line5 = 'Delete Appearance '..first_appear..' thru '..last_appear..' /nc /nu'
    local line6 = ''
    local condition_string = "Lua 'if Confirm(\"Delete Dimmer Phaser Engines "..name_prefix:gsub('%D*','').."?\") then; Cmd(\"Go macro "..macro_num.."\"); else Cmd(\"Off macro "..macro_num.."\"); end'"..' /nu'
...
function DIM_EFFECTS_ENGINE_CREATOR.create_layout(start_pos_x,start_pos_y,gap_x,gap_y,size_x,size_y,first_macro,first_sequence,groups,dpe_layout,first_appear,first_preset,first_matrick,index,name_prefix,DP)
    ...
    local col_num = 1
    local obj_index = #layout_pool[dpe_layout] + 1
    local x_count = index
    C('assign '..object_type..' '..pool_obj_num..' thru '..last_pool_obj..' at layout '..dpe_layout..' /nu')
    layout_pool[dpe_layout][obj_index]:Set('posx',65536+pos_x)
    layout_pool[dpe_layout][obj_index]:Set('posy',pos_y)
    layout_pool[dpe_layout][obj_index]:Set('action',14)
    layout_pool[dpe_layout][obj_index]:Set('visibilityobjectname',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilityindicatorbar',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilityborder',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilitybar',false)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONH',size_y)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONW',size_x)
    col_num = col_num + 1
    ...
    layout_pool[dpe_layout][obj_index]:Set('visibilityindicatorbar',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilitybar',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilityid',false)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONH',size_y)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONW',size_x)
    col_num = col_num + 1
    obj_index = obj_index + 1
    ...
    while pool_obj_num <= last_pool_obj do
        for col=1,2 do
            if layout_pool[dpe_layout][obj_index] then
                layout_pool[dpe_layout][obj_index]:Set('posx',65536+pos_x)
                layout_pool[dpe_layout][obj_index]:Set('posy',pos_y)
                layout_pool[dpe_layout][obj_index]:Set('visibilityobjectname',false)
                layout_pool[dpe_layout][obj_index]:Set('visibilityborder',false)
                ...
                layout_pool[dpe_layout][obj_index]:Set('CUSTOMTEXTTEXT',name)
                layout_pool[dpe_layout][obj_index]:Set('CUSTOMTEXTSIZE',12)
                layout_pool[dpe_layout][obj_index]:Set('POSITIONH',size_y)
                layout_pool[dpe_layout][obj_index]:Set('POSITIONW',size_x)
                pos_x = pos_x + gap_x
                obj_index = obj_index + 1
    ...
    layout_pool[dpe_layout][obj_index]:Set('posx',65536+pos_x)
    layout_pool[dpe_layout][obj_index]:Set('posy',pos_y)
    layout_pool[dpe_layout][obj_index]:Set('visibilityobjectname',true)
    layout_pool[dpe_layout][obj_index]:Set('visibilityborder',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilityindicatorbar',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilitybar',false)
    layout_pool[dpe_layout][obj_index]:Set('visibilityid',false)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONH',size_y)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONW',size_x*2+gap_x-size_x)
    col_num = col_num + 1
    obj_index = obj_index + 1
    ...
    x_count = index
    y_count = 6
    C('store layout '..dpe_layout..'.'..pool_obj_num..' thru '..last_pool_obj..' /nu')
    for i=1,6 do -- 6 for 6 labels
        ...
        layout_pool[dpe_layout][obj_index]:Set('CUSTOMTEXTTEXT',name)
        layout_pool[dpe_layout][obj_index]:Set('CUSTOMTEXTSIZE',20)
        layout_pool[dpe_layout][obj_index]:Set('VISIBILITYBORDER',false)
        layout_pool[dpe_layout][obj_index]:Set('POSITIONH',size_y)
        layout_pool[dpe_layout][obj_index]:Set('POSITIONW',size_x)
        pos_y = pos_y - gap_y
    ...
    layout_pool[dpe_layout][obj_index]:Set('posx',65536+pos_x)
    layout_pool[dpe_layout][obj_index]:Set('posy',pos_y)
    layout_pool[dpe_layout][obj_index]:Set('visibilityborder',true)
    layout_pool[dpe_layout][obj_index]:Set('POSITIONW',math.floor(gap_x*8.3))
    if #groups < 12 then
        layout_pool[dpe_layout][obj_index]:Set('POSITIONH',math.floor(gap_y*8.3))
    else
        layout_pool[dpe_layout][obj_index]:Set('POSITIONH',math.floor(gap_y*math.ceil(#groups/2)+gap_y*2.2))
    end
    pos_y = pos_y - gap_y
    col_num = col_num + 1
    obj_index = obj_index + 1
...
function DIM_EFFECTS_ENGINE_CREATOR.get_free_page(DP)
...
function DIM_EFFECTS_ENGINE_CREATOR.get_fixed_executors(DP)
    local all_execs = ObjectList('datapool '..DP..' page *.*')
    local is_fixed = CmdObj().isfixed
    local fixed_execs = {}
    for _,exec in ipairs(all_execs) do
        if is_fixed(exec) == 1 then
            table.insert(fixed_execs,exec)
        end
    end
    return fixed_execs
end

function DIM_EFFECTS_ENGINE_CREATOR.fix_unfix_execs(execs)
    for _,exec in pairs(execs) do
        Cmd('fix '..exec:ToAddr())
    end    
end

function DIM_EFFECTS_ENGINE_CREATOR.main_function_dimmer_phaser_engines()
    -- require 'gma3_debug'()
    local data_pool
    ...
    local number_of_dimmer_phasers = 4
    local groups_selected
    local number_of_groups
    local dpe_layout_number
    local generated
    ...
    local selected_page = DIM_EFFECTS_ENGINE_CREATOR.get_free_page(data_pool)
    local suggested
    local amount
    --------------- getting user input --------------
    groups_selected = DIM_EFFECTS_ENGINE_CREATOR.get_groups(data_pool)
    ...
    if #groups_selected ~= 0 then
        ...
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(amount,all1_presets_pool)
        gl_first_phaser_preset = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('first Phaser Preset',suggested,amount,all1_presets_pool)
        ----------------------------- getting first sequence number
        amount = 1*engines_amount -- 1 sequence for engine
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(amount,sequence_pool)
        gl_first_sequence = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('first Sequence',suggested,amount,sequence_pool)
        ------------------- getting layout number
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(1,layouts_pool)
        dpe_layout_number = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('Layout number',suggested,1,layouts_pool)
        ...
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(amount,appearances_pool)
        gl_first_appear = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('first Appearance',suggested,amount,appearances_pool)
        ------------------------------- getting first matricks
        amount = 1*engines_amount
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(amount,matricks_pool)
        gl_first_matricks = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('first MAtricks',suggested,amount,matricks_pool)
        ----------------------- getting first macro 
        ...
        suggested = DIM_EFFECTS_ENGINE_CREATOR.get_first_pool_num(amount,macro_pool)
        gl_first_macro = DIM_EFFECTS_ENGINE_CREATOR.check_selected_pool_num('first Macro',suggested,amount,macro_pool)
        
        ------------------- getting symbol number
        -- suggested = dpe_get_first_pool_num(17,symbols_pool) ------------- 17 - amount of symbol filenames
        -- gl_first_symbol = pp_check_selected_pool_num('first Symbol',suggested,17,symbols_pool)
        ---------------------------[[ Plugin started ]]-------------------------
        C('cleara /nu')
        ----------------------------------------
        local declared_timings = DIM_EFFECTS_ENGINE_CREATOR.declare_timing_values()
        gl_name_prefix = DIM_EFFECTS_ENGINE_CREATOR.create_name_prefix(data_pool)
        local start_pos_x = gl_start_pos_x
        local start_pos_y = gl_start_pos_y
        C('store layout '..dpe_layout_number..' "'..gl_name_prefix..'layout" /nu')
        C('select layout '..dpe_layout_number..' /nu')
        C('store page '..selected_page..' /nu')
        local fixed_execs = DIM_EFFECTS_ENGINE_CREATOR.get_fixed_executors(data_pool)
        DIM_EFFECTS_ENGINE_CREATOR.fix_unfix_execs(fixed_execs)
        DIM_EFFECTS_ENGINE_CREATOR.create_appearances(0,gl_first_appear,0,declared_timings,0,gl_name_prefix,0,data_pool)
        DIM_EFFECTS_ENGINE_CREATOR.import_predefined_phasers(all1_presets_pool,gl_first_phaser_preset,gl_name_prefix)
        ...
        for index=1, engines_amount do
            DIM_EFFECTS_ENGINE_CREATOR.create_matricks(first_loop_matricks,gl_name_prefix,index)
            DIM_EFFECTS_ENGINE_CREATOR.create_sequence(first_loop_squence,#groups_selected,gl_first_appear,gl_first_phaser_preset,first_loop_matricks,sequence_pool,selected_page,gl_name_prefix,index,data_pool)
            DIM_EFFECTS_ENGINE_CREATOR.create_macros(groups_selected,first_loop_macro,first_loop_squence,first_loop_matricks,gl_first_appear,gl_first_phaser_preset,declared_timings,gl_name_prefix,index,data_pool)
            DIM_EFFECTS_ENGINE_CREATOR.create_layout(start_pos_x,start_pos_y,gl_gap_x,gl_gap_y,gl_size_x,gl_size_y,first_loop_macro,first_loop_squence,groups_selected,dpe_layout_number,gl_first_appear,gl_first_phaser_preset,first_loop_matricks,index,gl_name_prefix,data_pool)
        ...
        DIM_EFFECTS_ENGINE_CREATOR.create_delete_macro(gl_first_macro,0,gl_first_sequence,number_of_dimmer_phasers,#groups_selected,dpe_layout_number,gl_first_appear,0,gl_first_phaser_preset,gl_first_matricks,1,1,engines_amount,gl_name_prefix,data_pool)
        DIM_EFFECTS_ENGINE_CREATOR.fix_unfix_execs(fixed_execs)
    else
...
return DIM_EFFECTS_ENGINE_CREATOR.main_function_dimmer_phaser_engines