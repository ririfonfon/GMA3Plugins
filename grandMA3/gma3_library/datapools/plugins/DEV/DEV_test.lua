local function main()
    ...
    local TLay = Root().ShowData.DataPools[pool_construct].Layouts:Children()
    ...
    local SelectedGrp = { 'PIXEL RGB', 'SPOT CONTRE', 'MEGA POINTE SOL', 'RIVALE UP', 'BMFL', 'FLOOR PLATE STRIKE',
        'UP PLATE STRIKE' }
    local SelectedGrpNo = { '1', '2', '3', '4', '5', '6', '7' }
    local SeqNrStart = 2
    local MacroNrStart = 1
    local MatrickNr = Root().ShowData.DataPools[pool_construct].Matricks:Children()
    local MatrickNrStart = 1
    local NaLay = "Colors"
    local TLayNr = 2
