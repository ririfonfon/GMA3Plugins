...
function labelling_objects_layout(last_obj,size_x,size_y,gap_x,gap_y,start_pos_x,start_pos_y,cp_layout,amount_of_groups,presets,in_declared_timings,progress_bar_time,DP)
    ...
    while col_num <= x_count do
        ...
        layout_pool[color_layout][obj_num]:Set('POSITIONH',size_x)
        layout_pool[color_layout][obj_num]:Set('POSITIONW',size_y)
        layout_pool[color_layout][obj_num]:Set(custom_text_key,tostring(ColorPresetPool[presets[preset_i]].name):gsub('CP%d+_',''))
        layout_pool[color_layout][obj_num]:Set(custom_text_size_key,20)
