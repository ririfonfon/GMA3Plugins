...
local function Main(displayHandle)
    ...
    local list = false
    -- local DataPoollist = ShowData().DataPool:Children()
    local FixtureGroups = DataPool().Groups:Children()
    local PT_preset = DataPool().PresetPools.Position:Children()
    local SelectedGrp = {}
    local SelectedGrpNo = {}
    local SelGrp
    ...
    local popuplists = {
        Grp_Select      = {},
        PT_Select       = {},
        Name_Select     = { 'Phaser PT', 'Phaser 1234 PT', 'Phaser 1234 PT', '1234 PT', 'OneToFour PT', 'OTF PT' },
    ...
    if list == false then
        ...
        for k in ipairs(PT_preset) do
            table.insert(popuplists.PT_Select, "'" .. PT_preset[k].name .. "'")
        end
    ...
    input9Button.Name = 'PT_Select'
    input9Button.Text = "Please select PT"
