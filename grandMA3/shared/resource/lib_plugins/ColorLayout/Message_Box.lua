...
function Message_Box(tln,nl,sns,mns,an,mcl,fg,sg,sgn,sgnr)
    ...
    for k in ipairs(FixtureGroups) do
        -- table.insert(TGrpChoise,({name ='"' .. FixtureGroups[k].name .. '"' , state = false }) )
        table.insert(TGrpChoise,{name = "State A", state = true, group = 1})

        E(FixtureGroups[k].name)
    end

    for k in ipairs(ColGels) do
        ChoGel[ColGels[k].name] = ColGels[k].no
    end
    ...
    local options = {
        ...
        states = {{TGrpChoise}},
        selectors ={{name = "Gels", selectedValue = 1, values = ChoGel, type=0}}
    }

    local result = MessageBox(options)
